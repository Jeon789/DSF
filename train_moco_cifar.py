...
def knn(net, memory_data_loader, test_data_loader, start_time, epoch, args):
    ...
    with torch.no_grad():
        ...
        # [N]
        if args.dataset in ['CIFAR10', 'CIFAR100', 'IN100']:
            targets = memory_data_loader.dataset.targets  # np.array -> list
        elif args.dataset == 'stl10':
            targets = memory_data_loader.dataset.labels.tolist()  # list
        else:
            targets = memory_data_loader.dataset._labels
        ...
        write_log(os.path.join(args.results_dir, "log.txt"), msg + "\n")
...
if __name__ == "__main__" :
    ...
    # create model
    model = ModelMoCo(dim=args.moco_dim, K=args.moco_k, m=args.moco_m, T=args.moco_t, arch=args.arch, bn_splits=args.bn_splits, symmetric=args.symmetric).cuda()
    print('Model done')
