...
parser.add_argument('-b', '--batch-size', default=4096, type=int, metavar='N', help='mini-batch size (default: 4096), total batch size of all GPUs')
...
parser.add_argument('--moco-m', default=0.99, type=float, help='momentum for key encoder update (default: 0.99)')
parser.add_argument('--moco-m-cos', action='store_true', help='cosine schedule for momentum increasing to 1')
...
# Optimizer & training enhancements
parser.add_argument('--optimizer', default='lars', type=str, choices=['lars', 'adamw', 'sgd'], help='optimizer type (default: lars)')
parser.add_argument('--warmup-epochs', default=10, type=int, metavar='N', help='number of warmup epochs')
parser.add_argument('--crop-min', default=0.08, type=float, help='minimum random crop scale (default: 0.08)')

# Dataset & results
parser.add_argument('--results_dir', default='', type=str, metavar='PATH', help='directory to save logs/checkpoints')
parser.add_argument('--dataset', default='IN100', type=str, choices=['IN', 'IN100'])

# kNN monitoring & misc
parser.add_argument('--test_freq', default=10, type=int, help='evaluation frequency (epochs)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch.startswith('vit'):
        model = mocov3.MoCo_ViT(
            partial(vits.__dict__[args.arch], stop_grad_conv1=args.stop_grad_conv1),
            args.moco_dim, args.moco_mlp_dim, args.moco_t)
    else:
        model = mocov3.MoCo_ResNet(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.rank == 0 and ((epoch+1) % args.test_freq == 0):
            if args.distributed:
